 ...
 * Date   : 5. October 2003
 * Only used to test the error handling of pygsl_lite.
 */
#include <Python.h>
...
trigger_save_state(PyObject *self, PyObject *args)
{
	...
	pygsl_lite_error("Triggered safe test!",
...
select_error_handler(PyObject *self, PyObject *args)
{
	...
	default:
		...
		pygsl_lite_error("Unknown error handler to be selected", __FILE__, line, status);
		goto fail;
...
trigger(PyObject *self, PyObject *args)
{
	 ...
	 * 
	 */ 
	pygsl_lite_error("Just a test to see what pygsl_lite is doing!",
