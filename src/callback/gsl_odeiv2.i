 ...
 *
*/
typedef gsl_odeiv2_step pygsl_lite_odeiv2_step;
typedef struct{
	gsl_odeiv2_control *control;
	PyArrayObject *scale;
	size_t dimension;
} pygsl_lite_odeiv2_control;
/* avoid name clashes */
typedef gsl_odeiv2_evolve pygsl_lite_odeiv2_evolve;
typedef struct{
	gsl_odeiv2_driver * driver;
	PyObject * sys;
	PyArrayObject * scale;
	size_t dimension;
} pygsl_lite_odeiv2_driver;

static int _pygsl_lite_odeiv2_system_set_function(pygsl_lite_odeiv2_system *self, PyObject * cb)
{
...
static int _pygsl_lite_odeiv2_system_set_jacobian(pygsl_lite_odeiv2_system *self, PyObject * cb)
{
	if (cb == Py_None){
...
static int _pygsl_lite_odeiv2_system_set_dimension(pygsl_lite_odeiv2_system *self, size_t dimension)
{
...
struct _pygsl_lite_odeiv2_steppers_require_jacobian{
...
static int _pygsl_lite_odeiv2_check_step_jacobian(const pygsl_lite_odeiv2_step *step, const gsl_odeiv2_system *dydt)
{
	...
	struct _pygsl_lite_odeiv2_steppers_require_jacobian *p, _require_jac[]= {
	...
	for (p = _require_jac; p->type != NULL; ++p){
		...
		default:
			line = __LINE__;
			status =  GSL_ESANITY;
			/* should that not be a pygsl_lite_error */
...
%rename(system) pygsl_lite_odeiv2_system;
typedef struct{
	%immutable;
}pygsl_lite_odeiv2_system;
...
%extend pygsl_lite_odeiv2_system{
	gsl_error_flag_drop set_func(PyObject * cb){
		return  _pygsl_lite_odeiv2_system_set_function(self, cb);
	}

	gsl_error_flag_drop set_jacobian(PyObject * cb){
		return  _pygsl_lite_odeiv2_system_set_jacobian(self, cb);
	}

	gsl_error_flag_drop set_dimension(size_t dimension){
		return _pygsl_lite_odeiv2_system_set_dimension(self, dimension);
	}

	pygsl_lite_odeiv2_system(PyObject * func, PyObject *jac, size_t dimension, PyObject *args){
		pygsl_lite_odeiv2_system *sys = NULL;
		int flag;

		sys = PyMem_NEW(pygsl_lite_odeiv2_system, 1);
		sys->params.py_func = NULL;
		sys->params.py_jac = NULL;
		sys->params.arguments = NULL;
		sys->dydt.function = PyGSL_odeiv_func;

		flag = _pygsl_lite_odeiv2_system_set_dimension(sys, dimension);
		if(flag != GSL_SUCCESS){
			goto fail;
		}

		flag = _pygsl_lite_odeiv2_system_set_function(sys, func);
		if(flag != GSL_SUCCESS){
			goto fail;
		}

		flag = _pygsl_lite_odeiv2_system_set_jacobian(sys, jac);
		if(flag != GSL_SUCCESS){
			goto fail;
		}
...
typedef struct{
	%immutable;
} pygsl_lite_odeiv2_step;


%extend pygsl_lite_odeiv2_step{
	pygsl_lite_odeiv2_step(const gsl_odeiv2_step_type * T, size_t dim){
		if(T == NULL){
			pygsl_lite_error("Type None/NULL not accepted", __FILE__, __LINE__ -1, GSL_EINVAL);
			return NULL;
		}
		return gsl_odeiv2_step_alloc(T, dim);
	}

	~pygsl_lite_odeiv2_step(void){
...
%extend pygsl_lite_odeiv2_control{
	pygsl_lite_odeiv2_control(double eps_abs, double eps_rel, double a_y, double a_dydt, PyObject *scale_abs){
		size_t dim= 0;
		PyArrayObject *sa = NULL;
		double * s_abs;
		pygsl_lite_odeiv2_control * r = NULL;

		r = (pygsl_lite_odeiv2_control *) PyMem_Malloc(sizeof(pygsl_lite_odeiv2_control));
		if(r == NULL){
			goto fail;
		}
		r->control = NULL;
	...
	~pygsl_lite_odeiv2_control(void){
	...
	PyObject * hadjust(pygsl_lite_odeiv2_step * s, PyObject * y_o, PyObject *yerr_o, PyObject * dydt_o, double h_in){
		...
		if(dim != self->dimension){
			pygsl_lite_error("dimension of step and control do not match!", __FILE__, __LINE__-1, GSL_EINVAL);
		}
...
typedef struct{
	%immutable;
} pygsl_lite_odeiv2_evolve;

%extend pygsl_lite_odeiv2_evolve{
	pygsl_lite_odeiv2_evolve(size_t dim){
		return gsl_odeiv2_evolve_alloc(dim);
	}

	~pygsl_lite_odeiv2_evolve(void){
	...
	PyObject *
	apply(pygsl_lite_odeiv2_control * con,
	      pygsl_lite_odeiv2_step * step,
	      const pygsl_lite_odeiv2_system * sys, const double t_in,
	      const double t1, const double h_in, PyObject * y_in_o)
	{
		...
		if(con == NULL){
			pygsl_lite_error("control object not set?", __FILE__, __LINE__, GSL_EINVAL);
			line = __LINE__ - 2; goto fail;
		}
		if(step == NULL){
			pygsl_lite_error("step object not set?", __FILE__, __LINE__, GSL_EINVAL);
			line = __LINE__ - 2; goto fail;
		}
		if(sys == NULL){
			pygsl_lite_error("sys object not set?", __FILE__, __LINE__, GSL_EINVAL);
			line = __LINE__ - 2; goto fail;
		}
		...
		if(sys->dydt.dimension != dim){
			pygsl_lite_error("sys dimension does not match evolve dimension", __FILE__, __LINE__, GSL_EINVAL);
			line = __LINE__ - 2; goto fail;
		}
	...
	PyObject *
	apply_fixed_step(pygsl_lite_odeiv2_control * con,
			 pygsl_lite_odeiv2_step * step, const pygsl_lite_odeiv2_system * sys,
...
%extend pygsl_lite_odeiv2_driver{
	~pygsl_lite_odeiv2_driver(void){
	...
	pygsl_lite_odeiv2_driver(PyObject *sys_o,
		...
		pygsl_lite_odeiv2_driver *d =NULL;
		double *scale_abs;
		...
		FUNC_MESS_BEGIN();
		flag = SWIG_ConvertPtr(sys_o, (void **) &sys, SWIGTYPE_p_pygsl_lite_odeiv2_system, 0 |  0 );
		if (!SWIG_IsOK(flag)) {
		...
		if(sys == NULL){
			...
			pygsl_lite_error("sys None/NULL not accepted", __FILE__, __LINE__ -1, GSL_EINVAL);
			goto fail;
		}
		if(T == NULL){
	...
	PyObject * apply_fixed_step(const double tin, const double h, const unsigned long int n, PyObject *y_o){
		...
		status = _pygsl_lite_odeiv2_check_step_jacobian(self->driver->s, self->driver->sys);
		if(GSL_SUCCESS != PyGSL_ERROR_FLAG(status)){
			line = __LINE__ - 2; goto fail;
		}
