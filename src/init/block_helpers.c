...
#include <pygsl_lite/old_numpy_compat.h>
#include <pygsl_lite/error_helpers.h>
#include <stdlib.h>
#include <stdio.h>
#include <pygsl_lite/profile.h>
 ...
 * pygsl_lite_profile_vector_transform_counter via the macro
 ...
 * counter pygsl_lite_profile_matrix_transform_counter via the macro
		    ...
		    PyGSL_array_index_t * stride_recalc)
{
     ...
     pygsl_lite_error("Can not convert the stride to a GSL stride",
		    ...
		    PyGSL_array_index_t *dimensions, int argnum, PyGSL_error_info * info)
{
     ...
     int i;
     int error_flag = GSL_ESANITY, line = -1, nd_test, type_num, array_flags;
     PyGSL_array_index_t  dim_temp, *dimensions_test;

     FUNC_MESS_BEGIN();
     if(!PyArray_Check((PyObject *) a_array)){
	  pygsl_lite_error("Did not recieve an array!", filename, __LINE__, GSL_ESANITY);
	  line = __LINE__ - 2;
     ...
     if(nd <  1 || nd > 2){
	  ...
	  line = __LINE__ - 2;
	  pygsl_lite_error("nd must either 1 or 2!", filename, __LINE__, GSL_ESANITY);
	  error_flag =  GSL_ESANITY;
	  goto fail;
     }

     nd_test = PyArray_NDIM(a_array);
     if (nd_test !=  nd){
	  DEBUG_MESS(3, "array->nd = %d\t nd = %d", nd_test, nd);
	  line = __LINE__ - 1;
	  sprintf(pygsl_lite_error_str, "I could not convert argument number % 3d."
		  " I expected a %s, but got an array of % 3d dimensions!\n", argnum,
		  (nd == 1) ? "vector" : "matrix", nd_test);

	  if (info){
	       info->error_description = pygsl_lite_error_str;
	       PyGSL_set_error_string_for_callback(info);
	  } else {
	       pygsl_lite_error(pygsl_lite_error_str, filename, __LINE__, GSL_EBADLEN);
	  }
     ...
     dimensions_test = PyArray_DIMS(a_array);
     ...
     if(PyArray_DATA(a_array) == NULL){
	  pygsl_lite_error("Got an array object were the data was NULL!", filename,
     ...
     /* a_array->descr->type_num */
     type_num = PyArray_TYPE(a_array);
     if(type_num == array_type){
	  DEBUG_MESS(4, "\t\tArray type matched! %d", 0);
     }else{
	  pygsl_lite_error("The array type did not match the spezified one!",
     ...
     array_flags = PyArray_FLAGS(a_array);
     if ((flag &  PyGSL_CONTIGUOUS) == 0){
	  ...
	  if(!(array_flags & CONTIGUOUS)){
	       DEBUG_MESS(3, "array->flags %d requested flags %d",
			 array_flags, flag);
	       pygsl_lite_error("The array is not contiguous as requested!", filename,
			   __LINE__, GSL_ESANITY);
	       error_flag = GSL_ESANITY;
	       line = __LINE__ - 3;
	       goto fail;
				       ...
				       int argnum)
{
     ...
     if(!tmp){
	  sprintf(pygsl_lite_error_str, "I could not convert argument number % 3d. to an integer.",
		  argnum);
	 PyErr_SetString(PyExc_TypeError, pygsl_lite_error_str);
	 return NULL;
     }
				      ...
				      int argnum)
{
     ...
     if(!PySequence_Check(src) || PySequence_Size(src) != 2){
	  sprintf(pygsl_lite_error_str, "I need a sequence of two elements as argument number % 3d",
		  argnum);
	 PyErr_SetString(PyExc_TypeError, pygsl_lite_error_str);
	 return NULL;
     ...
     for(i = 0; i<2; i++){
	  ...
	  if(!tmp){
	       sprintf(pygsl_lite_error_str, "I could not convert argument number % 3d. for dimension %3d to an integer.",
		       argnum, i);
	       PyErr_SetString(PyExc_TypeError, pygsl_lite_error_str);
	       return NULL;
	  }
	  ...
	  Py_DECREF(tmp);
	  if(dimensions[i] <= 0){
	       sprintf(pygsl_lite_error_str, "Argument number % 3d is % 10ld< 0. Its the size of the vector and thus must be positive!",
				...
				PyGSL_array_index_t p,  PyGSL_error_info * info)
{
     ...
     double tmp;
     char *myptr, *data=NULL;
     PyGSL_array_index_t *strides, i, j;
...
{
  int line = -1, status = GSL_EFAILED;
     PyArrayObject * r = NULL;
     PyGSL_array_index_t dim = 1;

     FUNC_MESS_BEGIN();
     if (PyGSL_GET_ARRAYTYPE(ainfo) != NPY_DOUBLE){
	  line = __LINE__ - 1;
	  status = GSL_ESANITY;
	  pygsl_lite_error("Array request for vector or double is not a double array!",
     ...
     if (PyGSL_GET_TYPESIZE(ainfo)  != sizeof(double)){
	  line = __LINE__ - 1;
	  status = GSL_ESANITY;
	  pygsl_lite_error("Type size passed for vector or double is not of sizeof(double)!",
		    __FILE__, line, status);
	  goto fail;
     }
     ...
     if(r == NULL){
	  ...
	  FUNC_MESS("=> FLOAT");
	  r = (PyArrayObject *) PyGSL_New_Array(1, &dim, NPY_DOUBLE);
	  if(r == NULL) {
	       line = __LINE__ - 2;
	       goto fail;
	  }
	  (*(double *)PyArray_DATA(r)) = v;
     }
     FUNC_MESS_END();
     return r;
		   ...
		   PyGSL_array_index_t *stride, PyGSL_error_info * info)
{
  ...
  int line=-1, tries, status = GSL_EFAILED;
     ...
     for(tries = 0; tries <2; ++tries){
	  ...
	  if(PyGSL_STRIDE_RECALC(strides[0], type_size, stride) == GSL_SUCCESS){
	       ...
	       if((flag & PyGSL_CONTIGUOUS) == 1){
		    ...
		    if(PyGSL_DEBUG_LEVEL() > 0){
			 if(*stride != 1){
			      line = __LINE__ - 1;
			      status = GSL_ESANITY;
			      pygsl_lite_error("Stride not one for a contiguous array!",
	  ...
	  DEBUG_MESS(2, "Stride recalc failed type size is  %ld, array stride[0] is %ld",
		     (long)type_size, (long)strides[0]);

	  if((flag & PyGSL_CONTIGUOUS) == 1){
	       line = __LINE__ - 1;
	       status = GSL_ESANITY;
	       pygsl_lite_error("Why does the stride recalc fail for a contigous array?",
			 filename, line, status);
	       goto fail;
	  } else {
		   ...
		   PyGSL_array_index_t *stride2, PyGSL_error_info * info)
{
     ...
     for(tries = 0; tries <2; ++tries){
	  ...
	  for(j = 0; j<2; ++j){
	       ...
	       if(PyGSL_STRIDE_RECALC(strides[j], type_size, stride) == GSL_SUCCESS){
		    /* everybody happy I hope! */
		    if((flag & PyGSL_CONTIGUOUS) == 1){
			 /*
			  *  just a check to see ... could be disabled later on when
			 ...
			 if(j == 1 && *stride != 1){
			      ...
			      pygsl_lite_error("Stride not one of a contiguous array!",
