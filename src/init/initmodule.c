 ...
 * created: March 2004
 * file: pygsl_lite/src/initmodule.c
 * $Id$
 *
 * Changes:
 *     7. October 2003:
 *     Removed the error handler from this file. It is now in 
 *     Lib/error_helpers.c Each module must call swig_init_pygsl_lite()
...
static const char pygsl_lite_debug_name[] = "pygsl_lite_debug";
#define _PyGSL_API_CAP_NAME pygsl_lite_api_name
#define _PyGSL_DEBUG_CAP_NAME pygsl_lite_debug_name
 ...
 */
static char pygsl_lite_error_str[512];
...
PyGSL_set_debug_level(PyObject *self, PyObject *args)
{
     ...
     pygsl_lite_debug_level = tmp;
     max = PySequence_Size(debuglist);
     DEBUG_MESS(3, "Setting debug level to %d for %d modules", pygsl_lite_debug_level, max);
     for(i = 0; i < max; ++i){
     ...
     status = GSL_EUNIMPL;
     pygsl_lite_error("PyGSL was not compiled with DEBUG = 1; Can not set DEBUG level!",
...
PyGSL_get_debug_level(PyObject *self, PyObject *args)
{
     int tmp;
#if DEBUG == 1
     tmp = (int) pygsl_lite_debug_level;
#else
     tmp = DEBUG;
#endif
...
static const char pygsl_lite_init_set_debug_level_doc[] =
  "Set the internal debug level\n"
  "\n"
  "see :py:func:`pygsl_lite.set_debug_level` for details\n";


static const char pygsl_lite_init_get_debug_level_doc[] =
...
static const char eh_state_reset_doc[] =
  ...
  "\n"
  "User access best using: :py:data:`pygsl_lite.errors.error_safe_state`\n";
...
static PyMethodDef initMethods[] = {
     {"get_debug_level", PyGSL_get_debug_level, METH_NOARGS, pygsl_lite_init_get_debug_level_doc},
     {"set_debug_level", PyGSL_set_debug_level, METH_VARARGS, pygsl_lite_init_set_debug_level_doc},
     ...
     {"error_handler_state_reset", PyGSL_pygsl_lite_error_handler_save_reset, METH_NOARGS, eh_state_reset_doc},
...
static const char pygsl_lite_init_m_doc[] =
  "pygsl_lite.init module\n"
"\n"
"This module is a collection of internal conversion function. These functions\n"
"typically need not be accessed by the pygsl_lite user. All functionality is wrapped\n"
"by appropriate functions and exported to the user at a higher level\n"
"\n"
"WARNING:\n"
"    Please note the functions given here change the total behaviour of pygsl_lite.\n"
...
#endif /* PyGSL_PY3K */
{
...
#else /* PyGSL_PY3K */
  m = Py_InitModule("pygsl_lite.init", initMethods);
#endif /* PyGSL_PY3K */
  ...
  if(m == NULL){
       fprintf(stderr, "I could not init pygsl_lite.init!");
       return RETVAL;
  }

  d = PyModule_GetDict(m);
  if(d == NULL){
       fprintf(stderr, "I could not get the module dict for  pygsl_lite.init!");
       return RETVAL;
  }
  ...
  if(version == NULL){
       fprintf(stderr, "I could not create the version string for pygsl_lite.init!");
       return RETVAL;
  }
  if(PyDict_SetItemString(d, "compiled_gsl_version", version) != 0){
       fprintf(stderr, "I could not add the compile version string to the module dict of pygsl_lite.init!");
       return RETVAL;
  }
  ...
  if(PyDict_SetItemString(d, "run_gsl_version", version) != 0){
       fprintf(stderr, "I could not add the run version string to the module dict of pygsl_lite.init!");
       return RETVAL;
  }

  date = PyGSL_string_from_string(DATE " " TIME);
  if(version == NULL){
       fprintf(stderr, "I could not create the date string for pygsl_lite.init!");
       return RETVAL;
  }
  if(PyDict_SetItemString(d, "compile_date", date) != 0){
       fprintf(stderr, "I could not add the date version string to the module dict of pygsl_lite.init!");
       return RETVAL;
  }
